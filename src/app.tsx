...
async function waitForElement(selector, maxAttempts = 50, interval = 100) {
  ...
  ); // Throw error if element not found within attempts
}
...
async function createWebMVideo() {
  try {
    ...
    ); // Get the left library video size
    ...
    let targetElementSelector =
      selectedPosition === "Bottom (Player)"
        ? bottomPlayerClass
        : leftLibraryClass;
    let elementStyles =
      selectedPosition === "Bottom (Player)"
...
function calculateBetterBPM(danceability, energy, currentBPM) {
  ...
  const betterBPMForFasterSongs =
    settings.getFieldValue("catjam-webm-bpm-method-faster-songs") !==
    "Track BPM";
