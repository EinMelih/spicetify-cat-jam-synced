...
async function main() {
  ...
  if (CFM.getGlobal("activationTypes") !== "btns") {
    if (CFM.getGlobal("keyActivation") !== "def")
      Spicetify.Mousetrap.bind("t", openwithTV);
    if (CFM.getGlobal("keyActivation") !== "tv")
  ...
  function render() {
    ...
    container.classList.toggle(
      ...
      ) as Settings["verticalMonitorSupport"]) &&
  ...
  async function updateInfo() {
    ...
    if (CFM.get("showAlbum") !== "never") {
      ...
      if (
        albumURI?.startsWith("spotify:album:") &&
        CFM.get("showAlbum") === "date"
      ) {
  ...
  async function updateBackground(
    ...
    const settingValue = CFM.get(
      "backgroundChoice"
    ) as Settings["backgroundChoice"];
  ...
  async function updateThemeColor(imageURL: string) {
    if (
      ...
      container.classList.toggle(
        "themed-buttons",
        Boolean(CFM.get("themedButtons"))
      );
      container.classList.toggle(
        "themed-icons",
        Boolean(CFM.get("themedIcons"))
      );
  ...
  async function updateContext() {
    const ctxDetails = await Utils.getContext(translations[LOCALE]).catch(
    ...
    if (
      ctx_source.innerText.toLowerCase() !==
        `${ctxDetails!.ctxSource}`.toLowerCase() ||
  ...
  async function updateUpNext() {
    if (
      Spicetify.Player.data.duration - Spicetify.Player.getProgress() <=
        (CFM.get("upnextTimeToShow") as Settings["upnextTimeToShow"]) * 1000 +
          50 &&
      ...
      if (fsd_second_span.offsetWidth > fsd_next_tit_art.offsetWidth - 2) {
        switch (CFM.get("upNextAnim")) {
          ...
          default:
            ...
            animTime =
              (fsd_first_span.offsetWidth - fsd_next_tit_art.offsetWidth - 2) /
              0.05;
            // animTime= 3000*(fsd_first_span.offsetWidth/fsd_next_tit_art.offsetWidth)
            fsd_myUp.style.setProperty(
              "--translate_width_fsd",
              `-${
                fsd_first_span.offsetWidth - fsd_next_tit_art.offsetWidth + 5
              }px`
            );
  ...
  function updatePlayingIcon(evt: any) {
    if (evt.data.is_paused || evt.data.isPaused) {
      ...
      pausedIcon.classList.toggle(
        "hidden",
        CFM.get("titleMovingIcon") as boolean
      );
  ...
  function toggleInvert() {
    ...
    if (
      ...
      if (
        ...
        INVERTED[
          Spicetify.Player.data.item?.metadata?.album_uri?.split(":")[2]
        ] = false;
    } else {
      container.style.setProperty("--main-color", "0,0,0");
      container.style.setProperty("--contrast-color", "255,255,255");
  ...
  async function activate() {
    ...
    if (CFM.get("enableFullscreen"))
      await Utils.fullScreenOn()?.catch((err) => {});
    ...
    container.querySelector<HTMLElement>("#fsd-foreground")!.oncontextmenu =
      openConfig;
    container.querySelector<HTMLElement>("#fsd-foreground")!.ondblclick =
      deactivate;
    ...
    if (CFM.get("volumeDisplay") !== "never") {
      ReactDOM.render(
        <SeekableVolumeBar
          state={CFM.get("volumeDisplay") as Settings["volumeDisplay"]}
        />,
  ...
  function createOptions(
    ...
    if (!(configValue in options)) {
      if (key in DEFAULTS[CFM.getMode()]) {
        ...
        saveGlobalOption(
          key as keyof Config,
          configValue as Config[keyof Config]
        );
      }
    }
  ...
  function createToggle(
    ...
    if (toggle) {
      if (key in DEFAULTS)
        toggle.checked = CFM.getGlobal(key as keyof Config) as boolean;
      else toggle.checked = CFM.get(key as keyof Settings) as boolean;

      toggle.onchange = (evt) =>
        callback((evt?.target as HTMLInputElement)?.checked);
    }
    return settingCard;
  }
  ...
  function createInputElement(
    ...
    if (inputElement) {
      if (key in DEFAULTS)
        inputElement.value = CFM.getGlobal(key as keyof Config) as string;
      else inputElement.value = CFM.get(key as keyof Settings) as string;

      inputElement.oninput = (evt) =>
        callback((evt?.target as HTMLInputElement)?.value);
    }
    return settingCard;
  }
  ...
  function openConfig(evt: Event | null = null): void {
    ...
    configContainer.append(
      ...
      createToggle(
        translations[LOCALE].settings.showAllArtists,
        "showAllArtists"
      ),
      createToggle(translations[LOCALE].settings.trimTitle, "trimTitle"),
      createToggle(
        translations[LOCALE].settings.songChangeAnimation,
        "enableFade"
      ),
      ...
      createOptions(
        translations[LOCALE].settings.backgroundChoice.setting,
        {
          ...
          animated_album:
            translations[LOCALE].settings.backgroundChoice.animatedArt,
          dynamic_color:
            translations[LOCALE].settings.backgroundChoice.dynamicColor,
          static_color:
            translations[LOCALE].settings.backgroundChoice.staticColor,
      ...
      createOptions(
        translations[LOCALE].settings.backgroundColor.setting,
        {
          ...
          DESATURATED:
            translations[LOCALE].settings.backgroundColor.desaturated,
          LIGHT_VIBRANT:
            translations[LOCALE].settings.backgroundColor.lightVibrant,
          DARK_VIBRANT:
            translations[LOCALE].settings.backgroundColor.darkVibrant,
      ...
      createInputElement(
        ...
        (value) => {
          ...
          if (
            CFM.get("backgroundChoice") === "static_color" &&
            Utils.isModeActivated()
          ) {
            Utils.overlayBack();
  ...
  const extraBar = document.querySelector(EXTRA_BAR_SELECTOR)
    ?.childNodes[0] as HTMLElement;
  if (CFM.getGlobal("fsHideOriginal")) {
    if (
      (extraBar.lastChild as HTMLElement).classList.contains(
        "control-button"
      ) ||
  ...
  if (CFM.getGlobal("activationTypes") != "keys") {
    ...
    if (CFM.getGlobal("buttonActivation") !== "def") {
      ...
      if (document.querySelector(TOP_BAR_SELECTOR)) {
        ...
        document
          .querySelector(TOP_BAR_SELECTOR_GLOBAL_NAVBAR)
          ?.prepend(tvButton);
      }
